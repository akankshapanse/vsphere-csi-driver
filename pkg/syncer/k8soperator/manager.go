...
func NewManager(
	...
	if err := certmanagerv1.AddToScheme(mgr.GetScheme()); err != nil {
		return nil, fmt.Errorf("failed to set scheme for K8s operator for type %s. Err: %+v", certmanagerv1.SchemeGroupVersion.Group, err)
	}

	if err := snapshotmetadatav1beta1.AddToScheme(mgr.GetScheme()); err != nil {
		return nil, fmt.Errorf("failed to set scheme for K8s operator for type %s. Err: %+v", snapshotmetadatav1beta1.SchemeGroupVersion.Group, err)
	}
